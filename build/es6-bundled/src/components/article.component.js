...
const parser=new DOMParser,element=parser.parseFromString(htmlText,"text/html").documentElement;highlightCode(element,languages);return element};_exports.convertMarkdownToHtml=convertMarkdownToHtml;const prism_base_url="/node_modules/prismjs",highlightCode=/*#__PURE__*/function(){var _ref=babelHelpers.asyncToGenerator(function*(element,languages=[]){new Promise((res,rej)=>_require.default([`${prism_base_url}/prism.js`],res,rej)).then(()=>Promise.all([...languages.map(language=>new Promise((res,rej)=>_require.default([`${prism_base_url}/components/prism-${language}.js`],res,rej)))])).then(_=>Prism.highlightAllUnder(element))});return function highlightCode(_x){return _ref.apply(this,arguments)}}();_exports.highlightCode=highlightCode;var markdown_service={convertMarkdownToHtml:convertMarkdownToHtml,highlightCode:highlightCode};_exports.$markdownService=markdown_service;const styles=_myApp.css`
  time {
    font-style: italic;
    padding: 0 0 0 8px;
  }
