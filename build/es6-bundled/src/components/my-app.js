                               ...
                               */DomApi.prototype.replaceChild;/**
                                ...
                                */DomApi.prototype.removeAttribute;/**
                                 ...
                                 */DomApi.prototype.querySelectorAll;/** @type {?Node} */DomApi.prototype.parentNode;/** @type {?Node} */DomApi.prototype.firstChild;/** @type {?Node} */DomApi.prototype.lastChild;/** @type {?Node} */DomApi.prototype.nextSibling;/** @type {?Node} */DomApi.prototype.previousSibling;/** @type {?HTMLElement} */DomApi.prototype.firstElementChild;/** @type {?HTMLElement} */DomApi.prototype.lastElementChild;/** @type {?HTMLElement} */DomApi.prototype.nextElementSibling;/** @type {?HTMLElement} */DomApi.prototype.previousElementSibling;/** @type {!Array<!Node>} */DomApi.prototype.childNodes;/** @type {!Array<!HTMLElement>} */DomApi.prototype.children;/** @type {?DOMTokenList} */DomApi.prototype.classList;/** @type {string} */DomApi.prototype.textContent;/** @type {string} */DomApi.prototype.innerHTML;forwardMethods(DomApi.prototype,["cloneNode","appendChild","insertBefore","removeChild","replaceChild","setAttribute","removeAttribute","querySelector","querySelectorAll"]);forwardReadOnlyProperties(DomApi.prototype,["parentNode","firstChild","lastChild","nextSibling","previousSibling","firstElementChild","lastElementChild","nextElementSibling","previousElementSibling","childNodes","children","classList"]);forwardProperties(DomApi.prototype,["textContent","innerHTML"]);/**
                                                                    ...
                                                                    */const dom=function(obj){obj=obj||document;if(!obj.__domApi){let helper;if(obj instanceof Event){helper=new EventApi(obj)}else{helper=new DomApi(obj)}obj.__domApi=helper}return obj.__domApi};_exports.dom=dom;var polymer_dom={matchesSelector:matchesSelector,DomApi:DomApi,EventApi:EventApi,dom:dom,flush:flush$1,addDebouncer:enqueueDebouncer};_exports.$polymerDom=polymer_dom;const bundledImportMeta$1=babelHelpers.objectSpread({},meta,{url:new URL("../../node_modules/%40polymer/polymer/lib/legacy/legacy-element-mixin.js",meta.url).href});let styleInterface=window.ShadyCSS;/**
...
dispatch(updateDrawerState(!1))};_exports.navigate=navigate;const loadPage=page=>dispatch=>{const rootPath=/$^/,homePath=/home/,articlesPath=/articles/,articlePath=/article\/(.[^\/]+)$/,videosPath=/videos/;let match,routeData=()=>1<match.length?match.slice(1):null;if((match=page.match(homePath))||(match=page.match(rootPath))){new Promise((res,rej)=>_require.default(["./home.component.js"],res,rej)).then(bundle=>bundle&&bundle.$homeComponent||{});dispatch(updatePage("home",routeData()))}else if(match=page.match(articlesPath)){new Promise((res,rej)=>_require.default(["./articles.component.js"],res,rej)).then(bundle=>bundle&&bundle.$articlesComponent||{});dispatch(updatePage("articles",routeData()))}else if(match=page.match(articlePath)){new Promise((res,rej)=>_require.default(["./article.component.js"],res,rej)).then(bundle=>bundle&&bundle.$articleComponent||{});dispatch(updatePage("article",routeData()))}else if(match=page.match(videosPath)){new Promise((res,rej)=>_require.default(["./videos.component.js"],res,rej)).then(bundle=>bundle&&bundle.$videosComponent||{});dispatch(updatePage("videos",routeData()))}else{page="view404";new Promise((res,rej)=>_require.default(["./my-view404.js"],res,rej)).then(bundle=>bundle&&bundle.$myView404||{});dispatch(updatePage(page))}if(window.ga){window.ga("set","page",page)}// Scroll to the top of the screen.
...
`;class MenuBar extends connect(store)(LitElement){static get styles(){return[SharedStyles,styles]}menuButtonClicked(){store.dispatch(updateDrawerState(!0))}drawerOpenedChanged(e){store.dispatch(updateDrawerState(e.target.opened))}render(){return html$1`
    ...
    `}static get properties(){return{selected:{type:String},drawerOpened:{type:Boolean}}}stateChanged(state){this.drawerOpened=state.app.drawerOpened;switch(state.app.page){case"articles":case"article":this.selected="articles";break;case"videos":this.selected="videos";break;default:this.selected=state.app.page;}}}window.customElements.define("m-menu-bar",MenuBar);class SnackBar extends LitElement{static get properties(){return{active:{type:Boolean}}}static get styles(){return[css`
      ...
      <slot></slot>
    `}}window.customElements.define("snack-bar",SnackBar);const getContent=()=>fetch("content/index.json").then(response=>response.json()).then(content=>store.dispatch(contentReceived(content)));_exports.getContent=getContent;const getMarkdown=path=>{fetch(`${path}`).then(response=>response.blob()).then(blob=>{const reader=new FileReader;reader.readAsText(blob);reader.onload=e=>store.dispatch(markdownReceived(e.target.result))})};_exports.getMarkdown=getMarkdown;var content_service={getContent:getContent,getMarkdown:getMarkdown};_exports.$contentService=content_service;class MyApp extends connect(store)(LitElement){static get properties(){return{appTitle:{type:String},_page:{type:String},_drawerOpened:{type:Boolean},_snackbarOpened:{type:Boolean},_offline:{type:Boolean}}}static get styles(){return[css`
        :host {
          display: block;
          width: 100%;
      ...
      `]}renderCurrentPage(){switch(this._page){case"home":return html$1`
          <m-home class="page" active></m-home>
        `;case"articles":return html$1`
...
setPassiveTouchGestures(!0);getContent()}firstUpdated(){installRouter(location=>store.dispatch(navigate(decodeURIComponent(location.pathname))));installOfflineWatcher(offline=>store.dispatch(updateOffline(offline)));installMediaQueryWatcher(`(min-width: 460px)`,()=>store.dispatch(updateDrawerState(!1)))}updated(changedProps){if(changedProps.has("_page")){const pageTitle=this._page+" // mikerambl.es";updateMetadata({title:pageTitle,description:pageTitle// This object also takes an image property, that points to an img src.
