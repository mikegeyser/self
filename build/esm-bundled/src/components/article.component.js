...
const parser=new DOMParser,element=parser.parseFromString(htmlText,"text/html").documentElement;highlightCode(element,languages);return element},prism_base_url="/node_modules/prismjs",highlightCode=async(element,languages=[])=>{import(`${prism_base_url}/prism.js`).then(()=>Promise.all([...languages.map(language=>import(`${prism_base_url}/components/prism-${language}.js`))])).then(_=>Prism.highlightAllUnder(element))};var markdown_service={convertMarkdownToHtml:convertMarkdownToHtml,highlightCode:highlightCode};const styles=css`
  time {
    font-style: italic;
    padding: 0 0 0 8px;
  }
