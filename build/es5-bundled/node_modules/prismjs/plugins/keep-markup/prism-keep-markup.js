(function(self,document){if("undefined"===typeof self||!self.Prism||!self.document||!document.createRange){return}Prism.plugins.KeepMarkup=!0;Prism.hooks.add("before-highlight",function(env){if(!env.element.children.length){return}var pos=0,data=[],f=function f(elt,baseNode){var o={};if(!baseNode){// Clone the original tag to keep all attributes
...
env.keepMarkup=data}});Prism.hooks.add("after-highlight",function(env){if(env.keepMarkup&&env.keepMarkup.length){var walk=function walk(elt,nodeState){for(var i=0,l=elt.childNodes.length,child;i<l;i++){child=elt.childNodes[i];if(1===child.nodeType){// element
