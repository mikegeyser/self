;/*! showdown v 1.9.0 - 10-11-2018 */(function(){/**
 ...
 * @param {function} [func]
 * @returns {*}
 */showdown.subParser=function(name,func){"use strict";if(showdown.helper.isString(name)){if("undefined"!==typeof func){parsers[name]=func}else{if(parsers.hasOwnProperty(name)){return parsers[name]}else{throw Error("SubParser named "+name+" not registered!")}}}};/**
 ...
 */function validate(extension,name){"use strict";var errMsg=name?"Error in "+name+" extension->":"Error in unnamed extension",ret={valid:!0,error:""};if(!showdown.helper.isArray(extension)){extension=[extension]}for(var i=0;i<extension.length;++i){var baseMsg=errMsg+" sub-extension "+i+": ",ext=extension[i];if("object"!==babelHelpers.typeof(ext)){ret.valid=!1;ret.error=baseMsg+"must be an object, but "+babelHelpers.typeof(ext)+" given";return ret}if(!showdown.helper.isString(ext.type)){ret.valid=!1;ret.error=baseMsg+"property \"type\" must be a string, but "+babelHelpers.typeof(ext.type)+" given";return ret}var type=ext.type=ext.type.toLowerCase();// normalize extension type
if("language"===type){type=ext.type="lang"}if("html"===type){type=ext.type="output"}if("lang"!==type&&"output"!==type&&"listener"!==type){ret.valid=!1;ret.error=baseMsg+"type "+type+" is not recognized. Valid values: \"lang/language\", \"output/html\" or \"listener\"";return ret}if("listener"===type){if(showdown.helper.isUndefined(ext.listeners)){ret.valid=!1;ret.error=baseMsg+". Extensions of type \"listener\" must have a property called \"listeners\"";return ret}}else{if(showdown.helper.isUndefined(ext.filter)&&showdown.helper.isUndefined(ext.regex)){ret.valid=!1;ret.error=baseMsg+type+" extensions must define either a \"regex\" property or a \"filter\" method";return ret}}if(ext.listeners){if("object"!==babelHelpers.typeof(ext.listeners)){ret.valid=!1;ret.error=baseMsg+"\"listeners\" property must be an object but "+babelHelpers.typeof(ext.listeners)+" given";return ret}for(var ln in ext.listeners){if(ext.listeners.hasOwnProperty(ln)){if("function"!==typeof ext.listeners[ln]){ret.valid=!1;ret.error=baseMsg+"\"listeners\" property must be an hash of [event name]: [callback]. listeners."+ln+" must be a function but "+babelHelpers.typeof(ext.listeners[ln])+" given";return ret}}}}if(ext.filter){if("function"!==typeof ext.filter){ret.valid=!1;ret.error=baseMsg+"\"filter\" must be a function, but "+babelHelpers.typeof(ext.filter)+" given";return ret}}else if(ext.regex){if(showdown.helper.isString(ext.regex)){ext.regex=new RegExp(ext.regex,"g")}if(!babelHelpers.instanceof(ext.regex,RegExp)){ret.valid=!1;ret.error=baseMsg+"\"regex\" property must either be a string or a RegExp object, but "+babelHelpers.typeof(ext.regex)+" given";return ret}if(showdown.helper.isUndefined(ext.replace)){ret.valid=!1;ret.error=baseMsg+"\"regex\" extensions must implement a replace string or function";return ret}}}return ret}/**
 * Validate extension
 * @param {object} ext
 * @returns {boolean}
 ...
 * @param {string} a
 * @returns {boolean}
 */showdown.helper.isString=function(a){"use strict";return"string"===typeof a||babelHelpers.instanceof(a,String)};/**
 ...
 * @param {string} flags
 * @returns {string}
 */showdown.helper.replaceRecursiveRegExp=function(str,replacement,left,right,flags){"use strict";if(!showdown.helper.isFunction(replacement)){var repStr=replacement;replacement=function replacement(){return repStr}}var matchPos=rgxFindMatchPos(str,left,right,flags),finalStr=str,lng=matchPos.length;if(0<lng){var bits=[];if(0!==matchPos[0].wholeMatch.start){bits.push(str.slice(0,matchPos[0].wholeMatch.start))}for(var i=0;i<lng;++i){bits.push(replacement(str.slice(matchPos[i].wholeMatch.start,matchPos[i].wholeMatch.end),str.slice(matchPos[i].match.start,matchPos[i].match.end),str.slice(matchPos[i].left.start,matchPos[i].left.end),str.slice(matchPos[i].right.start,matchPos[i].right.end)));if(i<lng-1){bits.push(str.slice(matchPos[i].wholeMatch.end,matchPos[i+1].wholeMatch.start))}}if(matchPos[lng-1].wholeMatch.end<str.length){bits.push(str.slice(matchPos[lng-1].wholeMatch.end))}finalStr=bits.join("")}return finalStr};/**
 ...
 */showdown.helper.regexIndexOf=function(str,regex,fromIndex){"use strict";if(!showdown.helper.isString(str)){throw"InvalidArgumentError: first parameter of showdown.helper.regexIndexOf function must be a string"}if(!1===babelHelpers.instanceof(regex,RegExp)){throw"InvalidArgumentError: second parameter of showdown.helper.regexIndexOf function must be an instance of RegExp"}var indexOf=str.substring(fromIndex||0).search(regex);return 0<=indexOf?indexOf+(fromIndex||0):indexOf};/**
...
if("undefined"===typeof console){console={warn:function warn(msg){"use strict";alert(msg)},log:function log(msg){"use strict";alert(msg)},error:function error(msg){"use strict";throw msg}}}/**
 * Common regexes.
...
if("object"===babelHelpers.typeof(converterOptions)){for(var opt in converterOptions){if(converterOptions.hasOwnProperty(opt)){options[opt]=converterOptions[opt]}}}else{throw Error("Converter expects the passed parameter to be an object, but "+babelHelpers.typeof(converterOptions)+" was passed instead.")}if(options.extensions){showdown.helper.forEach(options.extensions,_parseExtension)}}/**
   * Parse extension
   * @param {*} ext
   * @param {string} [name='']
   * @private
...
throw Error("Extension loader error: Type unrecognized!!!");}}}/**
   ...
   */function listen(name,callback){if(!showdown.helper.isString(name)){throw Error("Invalid argument in converter.listen() method: name must be a string, but "+babelHelpers.typeof(name)+" given")}if("function"!==typeof callback){throw Error("Invalid argument in converter.listen() method: callback must be a function, but "+babelHelpers.typeof(callback)+" given")}if(!listeners.hasOwnProperty(name)){listeners[name]=[]}listeners[name].push(callback)}function rTrimInputText(text){var rsp=text.match(/^\s*/)[0].length,rgx=new RegExp("^\\s{0,"+rsp+"}","gm");return text.replace(rgx,"")}/**
   * Dispatch an event
   * @private
   ...
   * @param {{}} globals
   * @returns {string}
   */this._dispatch=function dispatch(evtName,text,options,globals){if(listeners.hasOwnProperty(evtName)){for(var ei=0,nText;ei<listeners[evtName].length;++ei){nText=listeners[evtName][ei](evtName,text,this,options,globals);if(nText&&"undefined"!==typeof nText){text=nText}}}return text};/**
   * Listen to an event
   * @param {string} name
   * @param {function} callback
...
if(-1<wholeMatch.search(/\(<?\s*>? ?(['"].*['"])?\)$/m)){url=""}else if(!url){if(!linkId){// lower-case and turn embedded newlines into spaces
...
url=url.replace(showdown.helper.regexes.asteriskDashAndColon,showdown.helper.escapeCharactersCallback);var result="<a href=\""+url+"\"";if(""!==title&&null!==title){title=title.replace(/"/g,"&quot;");//title = showdown.helper.escapeCharacters(title, '*_', false); // replaced line to improve performance
...
result+=" target=\"\xA8E95Eblank\""}result+=">"+linkText+"</a>";return result};// First, handle reference-style links: [link text] [id]
 ...
 *
 *         ... type <code>`bar`</code> ...
 */showdown.subParser("codeSpans",function(text,options,globals){"use strict";text=globals.converter._dispatch("codeSpans.before",text,options,globals);if("undefined"===typeof text){text=""}text=text.replace(/(^|[^\\])(`+)([^\r]*?[^`])\2(?!`)/gm,function(wholeMatch,m1,m2,m3){var c=m3;c=c.replace(/^([ \t]*)/g,"");// leading whitespace
...
if(!options.ghCodeBlocks){return text}text=globals.converter._dispatch("githubCodeBlocks.before",text,options,globals);text+="\xA80";text=text.replace(/(?:^|\n)(?: {0,3})(```+|~~~+)(?: *)([^\s`~]*)\n([\s\S]*?)\n(?: {0,3})\1/g,function(wholeMatch,delim,language,codeblock){var end=options.omitExtraWLInCodeBlocks?"":"\n";// First parse the github code block
 ...
 */showdown.subParser("hashCodeTags",function(text,options,globals){"use strict";text=globals.converter._dispatch("hashCodeTags.before",text,options,globals);var repFunc=function repFunc(wholeMatch,match,left,right){var codeblock=left+showdown.subParser("encodeCode")(match,options,globals)+right;return"\xA8C"+(globals.gHtmlSpans.push(codeblock)-1)+"C"};// Hash naked <code>
 ...
 */showdown.subParser("hashHTMLSpans",function(text,options,globals){"use strict";text=globals.converter._dispatch("hashHTMLSpans.before",text,options,globals);function hashHTMLSpan(html){return"\xA8C"+(globals.gHtmlSpans.push(html)-1)+"C"}// Hash Self Closing tags
...
limit=0;while(/¨C(\d+)C/.test(repText)){var num=RegExp.$1;repText=repText.replace("\xA8C"+num+"C",globals.gHtmlSpans[num]);if(10===limit){console.error("maximum nesting of 10 spans reached!!!");break}++limit}text=text.replace("\xA8C"+i+"C",repText)}text=globals.converter._dispatch("unhashHTMLSpans.after",text,options,globals);return text});/**
...
item=item.replace(/^([-*+]|\d\.)[ \t]+[\S\n ]*/g,function(wm2){return"\xA8A"+wm2});// m1 - Leading line or
// Has a double return (multi paragraph) or
// Has sublist
if(m1||-1<item.search(/\n{2,}/)){item=showdown.subParser("githubCodeBlocks")(item,options,globals);item=showdown.subParser("blockGamut")(item,options,globals)}else{// Recursion for sub-lists:
...
text+="\xA80";if(globals.gListLevel){text=text.replace(/^(( {0,3}([*+-]|\d+[.])[ \t]+)[^\r]+?(¨0|\n{2,}(?=\S)(?![ \t]*(?:[*+-]|\d+[.])[ \t]+)))/gm,function(wholeMatch,list,m2){var listType=-1<m2.search(/[*+-]/g)?"ul":"ol";return parseConsecutiveLists(list,listType,!0)})}else{text=text.replace(/(\n\n|^\n?)(( {0,3}([*+-]|\d+[.])[ \t]+)[^\r]+?(¨0|\n{2,}(?=\S)(?![ \t]*(?:[*+-]|\d+[.])[ \t]+)))/gm,function(wholeMatch,m1,list,m3){var listType=-1<m3.search(/[*+-]/g)?"ul":"ol";return parseConsecutiveLists(list,listType,!1)})}// strip sentinel
...
}else if(0<=str.search(/\S/)){str=showdown.subParser("spanGamut")(str,options,globals);str=str.replace(/^([ \t]*)/g,"<p>");str+="</p>";grafsOut.push(str)}}/** Unhashify HTML blocks */end=grafsOut.length;for(i=0;i<end;i++){var blockText="",grafsOutIt=grafsOut[i],codeFlag=!1;// if this is a marker for an html block...
...
var re=ext.regex;if(!babelHelpers.instanceof(re,RegExp)){re=new RegExp(re,"g")}text=text.replace(re,ext.replace)}return text});/**
...
text+="\xA80";var replaceFunc=function replaceFunc(wholeMatch,linkId,url,width,height,blankLines,title){linkId=linkId.toLowerCase();if(url.match(/^data:.+?\/.+?;base64,/)){// remove newlines
...
txt+="\n<!-- -->\n";return txt.trim()});showdown.subParser("makeMarkdown.listItem",function(node,globals){"use strict";for(var listItemTxt="",children=node.childNodes,childrenLenght=children.length,i=0;i<childrenLenght;++i){listItemTxt+=showdown.subParser("makeMarkdown.node")(children[i],globals)}// if it's only one liner, we need to add a newline at the end
...
if("function"===typeof define&&define.amd){define(function(){"use strict";return showdown});// CommonJS/nodeJS Loader
}else if("undefined"!==typeof module&&module.exports){module.exports=showdown;// Regular Browser loader
}else{root.showdown=showdown}}).call(this);//# sourceMappingURL=showdown.js.map