/* This is pretty shamelessly copied from https://css-houdini.rocks/rough-boxes/, 
   and then adapted to fit my use cases. I make no claim to the following code,
   and all credit goes to @iamvdo and rough.js.
*/
...
class RoughDrawable {
  ...
  drawLine(ctx, x1, y1, x2, y2, existingPath) {
    ...
    ctx.bezierCurveTo(
      midDispX + x1 + (x2 - x1) * divergePoint + this.getOffset(-halfOffset, halfOffset),
...
class RoughUnderline extends RoughDrawable {
  constructor(x, y, width, height) {
    super(['x', 'y', 'width', 'height']);
    this.x = x;
    this.y = y;
  ...
  draw(ctx) {
    ...
    ctx.strokeStyle = this.stroke;
    ctx.lineWidth = this.strokeWidth;
    this.drawLine(ctx, this.x, this.height, this.width, this.height);
    ctx.restore();
  }
}
...
registerPaint(
  ...
  class {
    static get inputProperties() {
      return ['--rough-fill', '--rough-stroke-width', '--rough-stroke', '--rough-roughness'];
    }
    ...
    paint(ctx, geom, properties) {
      ...
      rect.strokeWidth = properties
        .get('--rough-stroke-width')
        .toString()
        .replace('px', '');
      rect.draw(ctx);
    }
...
registerPaint(
  ...
  class {
    static get inputProperties() {
      return ['--rough-stroke-width', '--rough-stroke', '--rough-roughness'];
    }

    paint(ctx, geom, properties) {
      const padding = 10;
      var rect = new RoughUnderline(padding, padding, geom.width - padding * 2, geom.height - padding * 2);
      rect.roughness = properties.get('--rough-roughness').toString();
      rect.fillStyle = 'solid';
      rect.stroke = properties.get('--rough-stroke').toString();
      rect.strokeWidth = properties
